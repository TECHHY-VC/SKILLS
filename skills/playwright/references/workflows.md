...
- Do not issue parallel commands against the same session. On Windows this can fail with `EADDRINUSE` on the named session pipe.
- For audit loops, prefer one session per flow and execute `open -> snapshot -> interact -> snapshot` sequentially.
...
- If complex `eval` commands fail in PowerShell, simplify the expression or switch to `snapshot` plus `screenshot` verification instead of fighting quote escaping.
